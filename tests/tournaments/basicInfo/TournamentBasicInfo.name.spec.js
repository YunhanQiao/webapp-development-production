...
test.describe("Tournament Name Tests", () => {
  ...
  test("TEST 1: Accept name with exactly 4 characters", async ({ page }) => {
    await page.locator("#name").fill("Test");
    await page.locator("#startDate").fill("2025-04-01");
    await page.locator("#endDate").fill("2025-04-01");

    await page.waitForTimeout(500);
    const lengthError = page.locator("text=/at least.*4|minimum.*4/i");
    await expect(lengthError).toHaveCount(0);

    console.log("✅ TEST 1 PASSED: Name with 4 characters accepted");
  });

  test("TEST 2: Accept name with more than 4 characters", async ({ page }) => {
    await page.locator("#name").fill("Spring Golf Championship 2025");
    await page.locator("#startDate").fill("2025-04-01");
    await page.locator("#endDate").fill("2025-04-01");
    ...
    console.log("✅ TEST 2 PASSED: Long name accepted");
  });

  // Short Name Generation Algorithm Tests
  test("TEST 3: Show 'New Tournament' as default header", async ({ page }) => {
    const header = page.locator("#tournamentFormHeader");
    await expect(header).toHaveText("New Tournament", { timeout: 5000 });

    console.log("✅ TEST 3 PASSED: Default header is 'New Tournament'");
  });

  test("TEST 4: Generate short name from two-word tournament name", async ({
    page,
  }) => {
    const header = page.locator("#tournamentFormHeader");
    ...
    console.log("✅ TEST 4 PASSED: Two-word name generates correct short name");
  });

  test("TEST 5: Generate short name from single-word tournament name", async ({
    page,
  }) => {
    const header = page.locator("#tournamentFormHeader");
    const nameField = page.locator("#name");
    ...
    console.log(
      "✅ TEST 5 PASSED: Single-word name generates correct short name",
    );
  });

  test("TEST 6: Generate short name from three-word tournament name", async ({
    page,
  }) => {
    const header = page.locator("#tournamentFormHeader");
    const nameField = page.locator("#name");
    ...
    console.log(
      "✅ TEST 6 PASSED: Three-word name generates correct short name",
    );
  });

  test("TEST 7: Generate short name from multi-word tournament name", async ({
    page,
  }) => {
    const header = page.locator("#tournamentFormHeader");
    const nameField = page.locator("#name");
    ...
    console.log(
      "✅ TEST 7 PASSED: Multi-word name generates correct short name",
    );
  });

  // Year Handling Tests
  test("TEST 8: Update short name year when start date year changes", async ({
    page,
  }) => {
    const header = page.locator("#tournamentFormHeader");
    const nameField = page.locator("#name");
    ...
    expect(headerText1).toMatch(/SLT25/);
    console.log("✅ TEST 8-a PASSED: Short name shows year 25 for 2025 date");

    await page.locator("#startDate").fill("2026-04-01");
    await page.waitForTimeout(500);
    const headerText2 = await header.textContent();
    expect(headerText2).toMatch(/SLT26/);
    console.log(
      "✅ TEST 8-b PASSED: Short name updates to year 26 for 2026 date",
    );

    await page.locator("#startDate").fill("2025-04-01");
    await page.waitForTimeout(500);
    const headerText3 = await header.textContent();
    expect(headerText3).toMatch(/SLT25/);
    console.log(
      "✅ TEST 8-c PASSED: Short name updates back to year 25 for 2025 date",
    );
  });

  test("TEST 9: Show two-digit year in short name", async ({ page }) => {
    const header = page.locator("#tournamentFormHeader");
    const nameField = page.locator("#name");
    ...
    const headerText = await header.textContent();
    expect(headerText).toMatch(/T.*25/);
    expect(headerText).not.toContain("2025");

    console.log("✅ TEST 9 PASSED: Shows two-digit year");
  });

  // Uniqueness Handling Tests
  test("TEST 10: Update short name when tournament name changes", async ({
    page,
  }) => {
    const header = page.locator("#tournamentFormHeader");
    const nameField = page.locator("#name");
    ...
    await page.waitForTimeout(500);
    await expect(header).toContainText("SO25", { timeout: 5000 });
    console.log("✅ TEST 10-a PASSED: 'Spring Open' generates 'SO25'");

    await nameField.fill("Summer Championship");
    await page.waitForTimeout(500);
    await expect(header).toContainText("SC25", { timeout: 5000 });
    await expect(header).not.toContainText("SO25");
    console.log("✅ TEST 10-b PASSED: 'Summer Championship' generates 'SC25'");

    await nameField.fill("Fall Tournament");
    await page.waitForTimeout(500);
    await expect(header).toContainText("FT25", { timeout: 5000 });
    console.log("✅ TEST 10-c PASSED: 'Fall Tournament' generates 'FT25'");
  });

  // Display and UI Tests
  test("TEST 11: Preserve name value during form interaction", async ({
    page,
  }) => {
    const nameField = page.locator("#name");
    ...
    console.log(
      "✅ TEST 11-a PASSED: Name value preserved after filling dates",
    );
    ...
    console.log(
      "✅ TEST 11-b PASSED: Name value preserved after filling tee time",
    );
  });
});
